...
애플리케이션 컴파일과 패키징을 Dockerfile을 통한 멀티 스테이지를 사용하여 도커와 컴포즈 외의 도그를 사용하지 않아도 된다.
즉 의존 모듈이 별도로 존재하지 않는 것이 컨테이너 CI의 주된 장점이다.
도커 허브, 깃헙 액션, 애저 데브옵스 등 주요 매니지드 빌드 서비스가 이를 지원한다.

책에서 CI 파이프라인을 실행하기 위해 젠킨스를 사용한다.

실습) p.330 참고

빌드 파이프라인 각 단계는 보통 비슷하다.
1. 배치 스크립트 실행
2. 실행된 도커 컴포즈가 실질적인 작업 담당

이러한 패턴은 빌드 서비스를 갈아타기 유리하다.
젠킨스에서 깃헙 액션으로 빌드 서비스 변경시 파이프라인 스크립트만 추가하면 된다.

젠킨스 빌드 모든 단계는 컨테이너에서 실행된다.
1. 검증 단계
2. 빌드 단계
3. 테스트 단계
4. 푸시 단계
...
실습) p.332 참고
- curl http://registry.local:5000/v2_catalog
